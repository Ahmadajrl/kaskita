...
const CONFIG = {
  ...
  GAS_URL: 'https://script.google.com/macros/s/AKfycbz4Ruy0eQO6-0OEpX5doUgvpd8JZw8M7J83HvRtAYHob4b604IZWypAsUALpPN9dc2H/exec',
...
async function doLogin() {
  ...
  if (!u || !p) {
    return toast('Username dan password wajib diisi', 'warning');
  }
  ...
  try {
    ...
    if (user) {
      ...
      lsSet(STORAGE_KEYS.SESSION, {
        user: u
      });

      await loginSuccess(u, true);
...
async function loadUserData(username) {
  ...
  try {
    ...
    const kas = await getData("kas");

    const pengeluaran =
      await getData("pengeluaran");

    state.kasData =
      kas.filter(x => x.owner === username);

    state.pengeluaranData =
      pengeluaran.filter(x => x.owner === username);
