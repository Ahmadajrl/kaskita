...
const CONFIG = {
  ...
  GAS_URL: 'https://script.google.com/macros/s/AKfycbzGkFYst0qD0SfLNUaysjsjnqeNZj2uhLeQkBvRYjXedhvmN4gcdLFc8AbcO3SaqxWk/exec',
...
// ================================================================
// KUNCI VERIFIKASI TAMBAH KAS
// Disimpan di localStorage dengan format: kaskita_verified_{username}
// Dibersihkan otomatis saat logout atau cache hilang
// ================================================================
const VERIFY_KEY_PREFIX = 'kaskita_tambahkas_verified_';

function getVerifyKey(username) {
  return VERIFY_KEY_PREFIX + (username || 'guest');
}

/** Cek apakah user sudah terverifikasi di sesi ini */
function isTambahKasVerified(username) {
  try {
    const raw = localStorage.getItem(getVerifyKey(username));
    if (!raw) return false;
    const data = JSON.parse(raw);
    // Token harus cocok dengan session saat ini
    const session = getSession();
    if (!session || data.sessionToken !== session.sessionToken) return false;
...
/** Tandai user sudah terverifikasi */
function setTambahKasVerified(username) {
  try {
    const session = getSession();
    const token = session?.sessionToken || Date.now().toString(36);
    localStorage.setItem(getVerifyKey(username), JSON.stringify({
      verified: true,
      sessionToken: token,
...
/** Hapus status verifikasi (saat logout) */
function clearTambahKasVerified(username) {
  try {
    localStorage.removeItem(getVerifyKey(username));
  } catch {}
}
...
// ================================================================
// [GANTI] SHA-256 + SALT HELPERS
// Letakkan menggantikan fungsi sha256() yang lama
// ================================================================
...
/** Buat salt acak 32-karakter hex */
...
/** Hash password dengan salt: SHA-256(salt + password) */
...
function setSession(s) {
  // Tambahkan sessionToken unik untuk mengikat verifikasi ke sesi ini
  if (!s.sessionToken) s.sessionToken = Date.now().toString(36) + Math.random().toString(36).slice(2);
...
function setDbStatus(status) {
  ...
  console.log("SET DB STATUS:", status);
  const texts = {
    online:'Google Sheets Terhubung ✓',
  ...
  if(tdot){
      tdot.className=`db-dot-sm ${status}`;
  }
  if(tlbl){
      tlbl.textContent=
          status==='online'
...
// ================================================================
// AUTH SYSTEM (LOGIN + REGISTER + LOGIN SUCCESS)
// Multi User + SHA256 Password + Google Apps Script Backend
// ================================================================


// ================================================================
// [GANTI] doLogin — verifikasi dengan salt
// Letakkan menggantikan fungsi doLogin() yang lama
// ================================================================
async function doLogin() {
  ...
  showOverlay(
      'Memeriksa kredensial di Google Sheets...'
  );
  try{
    const users =
    await gasGet('user');
...
// ================================================================
// [GANTI] doRegister — simpan dengan salt
// Letakkan menggantikan fungsi doRegister() yang lama
// ================================================================
async function doRegister() {
  ...
  if(!signature)
      return toast(
          'Upload tanda tangan terlebih dahulu',
          'warning'
      );

  showOverlay(
      'Mendaftarkan akun ke Google Sheets...'
  );
...
// ================================================================
// [TAMBAH BARU] FORGOT PASSWORD FLOW
// Tambahkan fungsi-fungsi ini di bawah doRegister()
// ================================================================

/** Buka modal lupa password */
function openForgotPassword() {
  document.getElementById('modalForgotPw').classList.remove('hidden');
  ...
  document.getElementById('fpNewPwConfirm').value = '';
}

/** Tutup modal lupa password */
function closeForgotPassword() {
  document.getElementById('modalForgotPw').classList.add('hidden');
}

/** Step 1: verifikasi username ada di database */
async function fpVerifyUser() {
  const u = document.getElementById('fpUser').value.trim();
  if (!u) return toast('Masukkan username kamu', 'warning');

  showOverlay('Mencari akun...');
  try {
    const users = await gasGet('user');
    const found = users.find(r => String(r.username).trim() === u);
    hideOverlay();

    if (!found) {
      return toast('Username tidak ditemukan', 'error');
    }

    // Simpan username yang ditemukan untuk step 2
    document.getElementById('fpVerifiedUser').value = u;
    document.getElementById('fpStep1').classList.add('hidden');
    document.getElementById('fpStep2').classList.remove('hidden');
    toast('Akun ditemukan. Silakan buat password baru.', 'success');
  } catch (e) {
    hideOverlay();
    toast('Gagal menghubungi database: ' + e.message, 'error');
  }
}

/** Step 2: reset password dengan salt baru */
async function fpResetPassword() {
  const u   = document.getElementById('fpVerifiedUser').value;
  const p   = document.getElementById('fpNewPw').value;
  const p2  = document.getElementById('fpNewPwConfirm').value;

  if (!p)           return toast('Password baru wajib diisi', 'warning');
  ...
  showOverlay('Mereset password...');
  try {
    const salt = generateSalt();
    const hash = await hashWithSalt(p, salt);
    ...
    hideOverlay();
    toast('Gagal reset password: ' + e.message, 'error');
  }
}
// ================================================================
// LOGIN SUCCESS
...
async function loginSuccess(username, isNew) {
  ...
  ['sidebarUsername', 'topbarUser'].forEach(id => {
    const el = document.getElementById(id);
...
async function loadAllData() {
  try {
    ...
    const [kas, pe, siswaRes] = await Promise.all([
    ...
    console.log(
        "TOTAL KAS:",
        state.kasData.length
    );

    console.log(
        "TOTAL FILTER:",
        state.filteredKas.length
    );
...
async function syncData() {
  ...
  try {
    ...
    // Jalankan Sistem Prediksi AI (Predictive Analytics)
    renderNotification();
    ...
    toast(
      'Data berhasil disinkronisasi dari Google Sheets',
      'success'
    );
...
// ================================================================
function doLogout() {
  clearTambahKasVerified(state.currentUser);
  clearSession();
...
// ================================================================
// VERIFIKASI TAMBAH KAS — Modal & Logika
// ================================================================

let verifCameraStream = null;

/** Buka modal verifikasi sebelum Tambah Kas */
function bukaModalVerifikasiKas(intendedPage = 'tambahKas') {
  state.intendedPage = intendedPage;
  const modal = document.getElementById('modalVerifikasiKas');
  if (!modal) return;
  // Reset tampilan
  document.getElementById('verifStep').classList.remove('hidden');
  document.getElementById('verifCameraContainer').style.display = 'none';
  document.getElementById('verifPreviewContainer').classList.add('hidden');
  document.getElementById('verifStatus').className = 'verif-status';
...
function tutupModalVerifikasiKas() {
  hentikanKameraVerif();
  document.getElementById('modalVerifikasiKas').classList.add('hidden');
}

/** Buka kamera untuk verifikasi */
async function bukaKameraVerif() {
  try {
    verifCameraStream = await navigator.mediaDevices.getUserMedia({
      video: { facingMode: 'environment', width: { ideal: 1280 }, height: { ideal: 720 } }
    });
    document.getElementById('verifCameraPreview').srcObject = verifCameraStream;
    document.getElementById('verifCameraContainer').style.display = 'block';
    document.getElementById('verifStep').classList.add('hidden');
  } catch(e) {
    toast('Kamera tidak dapat dibuka: ' + e.message, 'error');
  }
}

/** Hentikan kamera verifikasi */
function hentikanKameraVerif() {
  if (verifCameraStream) {
    verifCameraStream.getTracks().forEach(t => t.stop());
    verifCameraStream = null;
  }
  const cc = document.getElementById('verifCameraContainer');
  if (cc) cc.style.display = 'none';
}

/** Ambil foto dari kamera verifikasi */
async function ambilFotoVerif() {
  const video  = document.getElementById('verifCameraPreview');
  const canvas = document.getElementById('verifCameraCanvas');
  ...
  tampilkanPreviewVerif(dataURL);

  // Konversi ke blob lalu proses
  const blob = await fetch(dataURL).then(r => r.blob());
  const file = new File([blob], 'verifikasi.png', { type: 'image/png' });
  await prosesVerifikasiGambar(file, 'camera');
}

/** Tampilkan preview gambar verifikasi */
function tampilkanPreviewVerif(src) {
  const container = document.getElementById('verifPreviewContainer');
  const img       = document.getElementById('verifPreviewImg');
  if (img) img.src = src;
  if (container) container.classList.remove('hidden');
}

/** Handle upload file verifikasi */
async function onVerifFileChange(e) {
  const file = e.target.files[0];
  if (!file) return;
  const reader = new FileReader();
  reader.onload = ev => tampilkanPreviewVerif(ev.target.result);
  reader.readAsDataURL(file);
  await prosesVerifikasiGambar(file, 'upload');
  // reset input agar bisa upload ulang
  e.target.value = '';
}

/** Proses gambar verifikasi menggunakan SignatureScanner (Pencocokan Tanda Tangan) */
async function prosesVerifikasiGambar(file, source = 'upload') {
  if (!state.userSignature) {
    toast('Tanda tangan pengguna belum terdaftar. Silakan login ulang atau hubungi admin.', 'error');
    return;
  }

  // Tampilkan Hologram Scanner
  const base64Input = await fileToBase64(file);
  SignatureScanner.showScannerUI(base64Input);

  try {
    // Jalankan deteksi
    const similarity = await SignatureScanner.compareSignatures(state.userSignature, base64Input);
    console.log(`[VERIF KAS] Similarity Score: ${similarity}% (Source: ${source})`);
    
    // Threshold minimal 5% untuk kamera dan 25% untuk upload gambar
    const minThreshold = source === 'camera' ? 5 : 25;
    const isSuccess = similarity >= minThreshold;

    SignatureScanner.setResultUI(isSuccess, similarity);

    setTimeout(() => {
      SignatureScanner.hideScannerUI();

      if (isSuccess) {
        setTambahKasVerified(state.currentUser);
        toast('Tanda tangan cocok! Membuka halaman...', 'success');
        tutupModalVerifikasiKas();
        
        navigate(state.intendedPage || 'tambahKas');
      } else {
        toast(`Tanda tangan tidak cocok. (Kecocokan: ${similarity.toFixed(2)}%)`, 'error');
        document.getElementById('verifStep').classList.remove('hidden');
      }
    }, 2000); // Tahan UI sukses/gagal selama 2 detik sebelum ditutup

  } catch(e) {
    SignatureScanner.hideScannerUI();
    toast('Gagal memproses gambar: ' + e.message, 'error');
    document.getElementById('verifStep').classList.remove('hidden');
  }
}
...
function navigate(page, doAnim=true) {
  // Cek verifikasi sebelum masuk Tambah Kas atau Pengeluaran
  if (page === 'tambahKas' || page === 'pengeluaran') {
    if (!isTambahKasVerified(state.currentUser)) {
      bukaModalVerifikasiKas(page);
      return;
    }
  }
...
function populateSiswaSelect() {
  ...
  const selFilter = document.getElementById('filterRecentName');
  ...
  if(sel) {
      sel.innerHTML = '<option value="">-- Pilih Siswa --</option>' + optionsHtml;
  }
  if(selFilter) {
      const currentVal = selFilter.value;
      selFilter.innerHTML = '<option value="">-- Semua Nama --</option>' + optionsHtml;
...
function renderNotification(){
  if (typeof PredictiveModel !== 'undefined') {
    PredictiveModel.renderPrediksiUI(state.kasData);
  }
}
// ================================================================
// ANIMATED COUNTER
...
function renderKasTable(df) {
  ...
  if(!df.length) { tbody.innerHTML=`<tr><td colspan="8" class="empty-state-cell"><div class="empty-state"><i class="fa-solid fa-inbox"></i><p>Belum ada data kas</p></div></td></tr>`; return; }
...
async function simpanKas() {
  ...
  // SECURITY: Prevent bypass dari console dengan mengecek validitas token session
  if (!isTambahKasVerified(state.currentUser)) {
    toast('Akses ditolak! Anda belum melakukan verifikasi tanda tangan yang valid.', 'error');
    bukaModalVerifikasiKas();
    return;
  }
  ...
  let record = { id:genId(), nama, tanggal, kelas, jurusan, nextReminder: hitungReminder(tanggal), status, nominal, keterangan:ket, owner:state.currentUser, createdAt:new Date().toISOString() };

  if (typeof AIFraudSystem !== 'undefined' && !window._aiFraudKasProceed) {
      const aiResult = AIFraudSystem.analyzeTransaction({ type: 'kas', nominal: nominal }, state.kasData);
      record.riskScore = aiResult.riskScore;
      record.riskLevel = aiResult.riskLevel;
      
      if (aiResult.riskScore >= 70) {
          showAIFraudModal(aiResult, () => {
              window._aiFraudKasProceed = true;
              simpanKas();
          });
          return;
      }
  }
  window._aiFraudKasProceed = false;

  showOverlay(state.editingKasId ? 'Memperbarui data kas di Google Sheets...' : 'Menyimpan data kas ke Google Sheets...');
  try {
    if (state.editingKasId) {
      await gasPost({ action: 'delete', table: 'kas', id: state.editingKasId });
      record.id = state.editingKasId;
      const oldRec = state.kasData.find(r => r.id === state.editingKasId);
      if(oldRec) record.createdAt = oldRec.createdAt;
      
      await gasPost({ action: 'insert', table: 'kas', ...record });
      
      state.kasData = state.kasData.map(r => r.id === state.editingKasId ? record : r);
      state.editingKasId = null;
      const btnSimpan = document.querySelector('#page-tambahKas .btn-primary');
      if(btnSimpan) btnSimpan.innerHTML = '<i class="fa-solid fa-cloud-arrow-up"></i> Simpan';
    } else {
      await gasPost({
        action: 'insert',
        table: 'kas',
...
function hitungReminder(tanggalInput){

    const reminder = new Date(tanggalInput);
...
function renderRecentKas() {
  ...
  const filterEl = document.getElementById('filterRecentName');
  const filterName = filterEl ? filterEl.value : "";
  
  let recentData = [...state.kasData];
  if (filterName) {
      recentData = recentData.filter(r => (r.nama || r.Nama || r.siswa) === filterName);
  }
    ...
    <div class="recent-item">
      <div style="display:flex;justify-content:space-between">
        ...
        <div style="display:flex; align-items:center; gap:8px;">
          ...
          <button class="btn-icon ripple" style="color:var(--blue); padding:4px;" onclick="editKas('${r.id}')" title="Edit"><i class="fa-solid fa-pen"></i></button>
          <button class="btn-icon ripple" style="color:var(--red); padding:4px;" onclick="hapusKas('${r.id}')" title="Hapus"><i class="fa-solid fa-trash"></i></button>
        </div>
      </div>
      <div class="recent-item-meta">${fmtDate(r.createdAt || r.tanggal)} · ${esc(r.kelas)} · ${esc(r.jurusan)} · ${stsBadge(r.status)}</div>
    </div>`).join('');
}

function editKas(id) {
  const record = state.kasData.find(r => r.id === id);
  if(!record) return;
  state.editingKasId = id;
  
  document.getElementById('fNama').value = record.nama || '';
  document.getElementById('fTanggal').value = record.tanggal || '';
  document.getElementById('fKelas').value = record.kelas || '';
  document.getElementById('fJurusan').value = record.jurusan || '';
  document.getElementById('fStatus').value = record.status || 'Tepat Waktu';
  document.getElementById('fNominal').value = record.nominal || '';
  document.getElementById('fKeterangan').value = record.keterangan || '';
  
  const btnSimpan = document.querySelector('#page-tambahKas .btn-primary');
  if(btnSimpan) btnSimpan.innerHTML = '<i class="fa-solid fa-cloud-arrow-up"></i> Update Data';
...
async function confirmHapus() {
  const id   = String(state.deleteTarget);
  const type = state.deleteType;
  closeModal();
  showOverlay('Menghapus data dari Google Sheets...');
  try {
    const record = state[type === 'kas' ? 'kasData' : 'pengeluaran'].find(r => String(r.id) === id) || {};
    
    // Mengirim id, nama, dan tanggal sesuai instruksi agar backend membaca ketiga kriteria
    await gasPost({
      action: 'delete',
      table: type,
      id: id,
    ...
    if(type==='kas') {
      state.kasData = state.kasData.filter(r => String(r.id) !== id);
      state.filteredKas = state.filteredKas.filter(r => String(r.id) !== id);
      populateFilterBulan(); 
      populateSiswaSelect();
      renderDashboard();
      renderRecentKas();
      if (String(state.editingKasId) === id) {
          clearKasForm();
      }
    } else {
      state.pengeluaran = state.pengeluaran.filter(r => String(r.id) !== id);
      renderPengeluaran();
    }
    hideOverlay();
...
async function simpanPengeluaran() {
  ...
  if (!isTambahKasVerified(state.currentUser)) {
    toast('Akses ditolak! Anda belum melakukan verifikasi tanda tangan yang valid.', 'error');
    bukaModalVerifikasiKas('pengeluaran');
    return;
  }
  ...
  if (typeof AIFraudSystem !== 'undefined' && !window._aiFraudPengeluaranProceed) {
      const aiResult = AIFraudSystem.analyzeTransaction({ type: 'pengeluaran', nominal: nominal }, state.pengeluaran);
      record.riskScore = aiResult.riskScore;
      record.riskLevel = aiResult.riskLevel;
      
      if (aiResult.riskScore >= 70) {
          showAIFraudModal(aiResult, () => {
              window._aiFraudPengeluaranProceed = true;
              simpanPengeluaran();
          });
          return;
      }
  }
  window._aiFraudPengeluaranProceed = false;
...
// ================================================================
// AI FRAUD MODAL HANDLERS
// ================================================================
let pendingAIFraudCallback = null;

function showAIFraudModal(aiResult, callback) {
    const modal = document.getElementById('modalAIFraudWarning');
    const reasonsEl = document.getElementById('aiFraudReasons');
    const btnProceed = document.getElementById('btnAIFraudProceed');
    const descEl = document.querySelector('#modalAIFraudWarning .modal-desc');
    
    reasonsEl.innerHTML = aiResult.reasons.map(r => `<li>${esc(r)}</li>`).join('');
    
    if (aiResult.riskScore >= 70) {
        if (btnProceed) btnProceed.style.display = 'none';
        if (descEl) descEl.textContent = "Transaksi DIBLOKIR sepenuhnya oleh sistem AI karena anomali tinggi:";
    } else {
        if (btnProceed) btnProceed.style.display = 'inline-block';
        if (descEl) descEl.textContent = "Sistem AI mendeteksi potensi anomali pada transaksi ini:";
    }
    
    pendingAIFraudCallback = callback;
    modal.classList.remove('hidden');
}

function closeAIFraudModal() {
    document.getElementById('modalAIFraudWarning').classList.add('hidden');
    pendingAIFraudCallback = null;
    window._aiFraudKasProceed = false;
    window._aiFraudPengeluaranProceed = false;
    toast('Transaksi dibatalkan karena indikasi anomali tinggi.', 'info');
}

function proceedAIFraudTransaction() {
    document.getElementById('modalAIFraudWarning').classList.add('hidden');
    if (pendingAIFraudCallback) {
        pendingAIFraudCallback();
        pendingAIFraudCallback = null;
    }
}
...
function jalankanClustering() {
  ...
  // FILTER DATA BERDASARKAN OWNER LOGIN
  const dataOwner = state.kasData.filter(item =>
    String(item.owner || "").trim() === String(state.currentUser).trim()
  );

  console.log("DATA OWNER:");
  console.table(dataOwner);

  if (!dataOwner.length) {
    console.log("Tidak ada data milik owner ini");
    return;
  }

  const rekap = {};

  dataOwner.forEach(item => {
    ...
    rekap[nama].nominal += nominal;
    rekap[nama].frekuensi++;
  });

  const dataSiswa = Object.values(rekap);

  console.log("HASIL CLUSTER OWNER:");
  console.table(dataSiswa);

  prosesClustering(dataSiswa);
}
// ======================================================
// EVENT ICON NOTIFIKASI
// ======================================================
...
document.addEventListener(
  ...
  console.log("BLOK OCR DIMUAT");
  async function preprocessImage(fileOrCanvas){
  ...
  try{
    ...
    if (!state.userSignature) {
      toast("Tanda tangan pengguna belum terdaftar", "error");
      hideOverlay();
      return;
    }
    ...
    const similarity = await SignatureScanner.compareSignatures(state.userSignature, base64Input);
    console.log("[OCR] Similarity Score:", similarity);

    const valid = similarity >= 25;
    SignatureScanner.setResultUI(valid, similarity);

    setTimeout(() => {
      SignatureScanner.hideScannerUI();
      if(valid){
        isiFormOtomatis(text);
        toast("Data berhasil diverifikasi dan diekstrak", "success");
      } else {
        toast(`Tanda tangan OCR tidak cocok (${similarity.toFixed(2)}%)`, "error");
      }
    }, 2000);
...
document.addEventListener(
  ...
  toast(
  "Aplikasi berpindah fokus. Buka ulang kamera.",
  "warning"
  );
  ...
  async function ambilFoto(){
    ...
    // Panggil SignatureScanner langsung tanpa cekTandaTangan lama
    if (!state.userSignature) {
      toast("Tanda tangan pengguna belum terdaftar", "error");
      hideOverlay();
      return;
    ...
    const base64Input = canvas.toDataURL("image/png");
    SignatureScanner.showScannerUI(base64Input);
...
async function cekTandaTangan(file){

  console.log(
  "FUNGSI CEK DIPANGGIL"
  );
  ...
  *100;
  ...
  <1% = kosong
  1–3% = kemungkinan noise
  >3% = ada tulisan/tanda tangan
  */
...
async function preprocessImage(file){
  ...
  ctx.filter=
  "grayscale(100%) contrast(180%) brightness(130%)";
...
function isiFormOtomatis(text){
  ...
  async function deteksiAreaTandaTangan(file){
