...
function bukaModalVerifikasiKas() {
...
async function onVerifFileChange(e) {
  ...
  await prosesVerifikasiGambar(file, 'file');
  // reset input agar bisa upload ulang
  e.target.value = '';
}

/** Proses gambar verifikasi menggunakan SignatureScanner (Pencocokan Tanda Tangan) */
async function prosesVerifikasiGambar(file, inputSource = 'file') {
  if (!state.userSignature) {
    toast('Tanda tangan pengguna belum terdaftar. Silakan login ulang atau hubungi admin.', 'error');
    return;
  }

  // Tentukan threshold berdasarkan sumber gambar
  // - 'camera' : foto langsung kamera → threshold rendah (toleransi noise foto)
  // - 'file'   : upload dari galeri/file → threshold lebih ketat
  const THRESHOLD = inputSource === 'camera' ? 5 : 25;
  ...
  try {
    // Jalankan deteksi, teruskan sumber input ke scanner
    const similarity = await SignatureScanner.compareSignatures(state.userSignature, base64Input, inputSource);
    console.log(`[VERIF KAS] Source: ${inputSource} | Threshold: ${THRESHOLD}% | Similarity: ${similarity.toFixed(2)}%`);
    
    const isSuccess = similarity >= THRESHOLD;
    ...
    setTimeout(() => {
      ...
      if (isSuccess) {
        ...
        toast('Tanda tangan cocok! Membuka Tambah Kas...', 'success');
        tutupModalVerifikasiKas();
        
        // Navigasi ke tambahKas
        state.currentPage = 'tambahKas';
        document.querySelectorAll('.page').forEach(p => p.classList.remove('active'));
        const el = document.getElementById('page-tambahKas');
        if (el) el.classList.add('active');
        document.querySelectorAll('.nav-item[data-page], .bottom-nav-item[data-page]').forEach(n => {
          n.classList.toggle('active', n.dataset.page === 'tambahKas');
        });
        const el2 = document.getElementById('topbarTitle');
        if (el2) el2.textContent = 'Tambah Data Kas';
        closeSidebar();
        renderPage('tambahKas');
      } else {
...
function navigate(page, doAnim=true) {
  // Cek verifikasi sebelum masuk Tambah Kas
  if (page === 'tambahKas') {
