...
const CONFIG = {
  ...
  GAS_URL: 'https://script.google.com/macros/s/AKfycbyl9pr3FSbi_h0NJdX4tqIjdai04ykC_uS8SmKJhl5G847VuEr3iG6tuwStcWWjfiFS/exec',
...
async function doRegister() {
  ...
  try {
    ...
    await gasPost({
      action: 'insert',
      table : 'admin',
      data  : { id:genId(), username:u, password:hash, createdAt:new Date().toISOString() }
    });
    hideOverlay();
...
async function simpanKas() {
  ...
  try {
    await gasPost({ action:'insert', table:'kas', data:record });
...
async function confirmHapus() {
  ...
  try {
    await gasPost({ action:'delete', table:type, data:{ id } });
    if(type==='kas') {
...
async function simpanPengeluaran() {
  ...
  try {
    await gasPost({ action:'insert', table:'pengeluaran', data:record });
