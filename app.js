...
async function doLogin() {
  ...
  try {
    ...
    if (found.salt) {
      // Akun baru: hash dengan salt
      const hash = await hashWithSalt(p, String(found.salt).trim());
      passwordValid = hash === String(found.password).trim();
    } else {
      // Akun lama (tanpa salt): fallback ke SHA-256 polos
...
function cekReminderKas(){
    ...
    // reset jam agar hanya membandingkan tanggal
    hariIni.setHours(0,0,0,0);

    const reminder = state.kasData.filter(item=>{
        ...
        tanggalReminder.setHours(0,0,0,0);

        return tanggalReminder <= hariIni;
    });

    console.log("DATA PENGINGAT:");
...
function jalankanClustering() {
  ...
  console.log("DATA KAS:");
  console.table(state.kasData);

  if (!state.kasData || state.kasData.length === 0) {
    console.log("state.kasData kosong");
    return;
  }
  ...
  if (dataSiswa.length === 0) {
      console.log("Tidak ada data clustering");
      return;
  }
