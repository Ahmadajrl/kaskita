...
function cekReminderKas(){
  ...
  state.kasData.forEach(item=>{
      ...
      if(!nama || !item.nextReminder)
          return;

      const nextDate =
      new Date(item.nextReminder);

      // simpan reminder TERDEKAT
      if(
          !reminderAktif[nama] ||
  ...
  Object.values(
      ...
      const tanggalReminder =
      new Date(
          item.nextReminder
      );

      tanggalReminder.setHours(
          0,0,0,0
      );

      return tanggalReminder <= hariIni;
