...
function buildFaceMatcher() {
  ...
  if (labeledDescriptors.length > 0) {
    // Distance 0.40 is a strict threshold to avoid false positives (recognizing unregistered faces)
    AI_STATE.faceMatcher = new faceapi.FaceMatcher(labeledDescriptors, 0.40);
  } else {
    AI_STATE.faceMatcher = null;
  }
}
      ...
      if (detections && detections.length > 0) {
        ...
        if (AI_STATE.faceMatcher) {
          ...
          if (match.label !== 'unknown') {
            ...
            // Map 0.0 -> 100%, 0.40 -> 70% confidence (minimum acceptable)
            // formula: 100 - (distance * 75) -> 100 - (0.40 * 75) = 70
            const confidence = Math.max(0, Math.round(100 - (distance * 75)));
            
            console.log(`[Diagnostics] Distance: ${distance.toFixed(3)}, Confidence: ${confidence}%, Threshold: 0.40, Matched: ${match.label}`);

            statusEl.innerHTML = `<span style="color:#00ff88">Wajah Cocok: ${confidence}%</span>`;
            
            // Auto Capture if > 70% (which means distance <= 0.40)
            if (confidence >= 70) {
              AI_STATE.isScanning = false;
              stopCamera();
            ...
            console.log(`[Diagnostics] Distance: ${match.distance.toFixed(3)}, Confidence: Low, Threshold: 0.40, Matched: None`);
