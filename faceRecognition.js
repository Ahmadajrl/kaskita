...
function buildFaceMatcher() {
  ...
  if (labeledDescriptors.length > 0) {
    // Distance 0.50 is much more robust for varying lighting and angles.
    AI_STATE.faceMatcher = new faceapi.FaceMatcher(labeledDescriptors, 0.50);
  } else {
    AI_STATE.faceMatcher = null;
  }
}
      ...
      if (detections && detections.length > 0) {
        ...
        if (AI_STATE.faceMatcher) {
          ...
          if (match.label !== 'unknown') {
            ...
            // Map 0.0 -> 100%, 0.50 -> 70% confidence (minimum acceptable)
            // formula: 100 - (distance * 60) -> 100 - (0.50 * 60) = 70
            const confidence = Math.max(0, Math.round(100 - (distance * 60)));
            
            console.log(`[Diagnostics] Distance: ${distance.toFixed(3)}, Confidence: ${confidence}%, Threshold: 0.50, Matched: ${match.label}`);

            statusEl.innerHTML = `<span style="color:#00ff88">Wajah Cocok: ${confidence}%</span>`;
            
            // Auto Capture if > 70% (which means distance <= 0.50)
            if (confidence >= 70) {
              AI_STATE.isScanning = false;
              stopCamera();
            ...
            console.log(`[Diagnostics] Distance: ${match.distance.toFixed(3)}, Confidence: Low, Threshold: 0.50, Matched: None`);
