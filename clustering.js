...
async function prosesClustering(dataSiswa) {
    ...
    // Inisialisasi Centroid awal secara deterministik (berdasarkan penyebaran data fitur 'telat')
    // Ini menjamin hasil clustering selalu stabil/sama pada data yang sama
    let centroids = [];
    let initialIndices = [];
    
    // Mengurutkan indeks berdasarkan fitur 'telat' (indeks 0)
    ...
    for(let i=0; i<k; i++) {
        // Ambil titik secara merata: paling kecil, median, paling besar
        const targetIdx = Math.floor(i * (sortedIndices.length - 1) / Math.max(1, k - 1));
        const actualIdx = sortedIndices[targetIdx];
        
        if (!initialIndices.includes(actualIdx)) {
            initialIndices.push(actualIdx);
            centroids.push([...normalizedData[actualIdx]]);
        } else {
            // Fallback jika terjadi duplikasi
            let fallback = 0;
            while(initialIndices.includes(fallback) && fallback < normalizedData.length) fallback++;
            initialIndices.push(fallback);
            centroids.push([...normalizedData[fallback]]);
        }
    }
