...
const SignatureScanner = {
    ...
    /**
     * Threshold kelulusan berdasarkan sumber gambar:
     * - 'camera' : foto langsung dari kamera (toleransi tinggi karena noise foto)
     * - 'file'   : gambar dipilih dari file/galeri (toleransi lebih ketat)
     */
    THRESHOLD: {
        camera: 5,
        file: 25
    ...
    // Sumber input terakhir yang digunakan ('camera' | 'file')
    _lastInputSource: 'file',
     ...
     * @param {string} base64User   - Gambar tanda tangan referensi (dari data user)
     * @param {string} base64Input  - Gambar tanda tangan yang diinput
     * @param {string} inputSource  - Sumber input: 'camera' | 'file' (default: 'file')
     */
    compareSignatures: async function(base64User, base64Input, inputSource = 'file') {
        // Simpan sumber input agar bisa diakses oleh verify()
        this._lastInputSource = (inputSource === 'camera') ? 'camera' : 'file';
    ...
    /**
     * Verifikasi lengkap: bandingkan tanda tangan lalu tentukan lolos/gagal
     * berdasarkan threshold sesuai sumber input.
     *
     * @param {string} base64User   - Gambar tanda tangan referensi
     * @param {string} base64Input  - Gambar tanda tangan yang diinput
     * @param {string} inputSource  - 'camera' | 'file'
     * @returns {Promise<{passed: boolean, score: number, threshold: number, source: string}>}
     *
     * Contoh pemakaian:
     *   const result = await SignatureScanner.verify(refBase64, inputBase64, 'camera');
     *   if (result.passed) { ... }
     */
    verify: async function(base64User, base64Input, inputSource = 'file') {
        const source    = (inputSource === 'camera') ? 'camera' : 'file';
        const threshold = this.THRESHOLD[source];

        const score  = await this.compareSignatures(base64User, base64Input, source);
        const passed = score >= threshold;

        console.log(
            `[SCANNER VERIFY] Source: ${source} | Threshold: ${threshold}% | ` +
            `Score: ${score.toFixed(2)}% | ${passed ? 'LOLOS ✓' : 'GAGAL ✗'}`
        );

        this.setResultUI(passed, score);
        return { passed, score, threshold, source };
    }
};
